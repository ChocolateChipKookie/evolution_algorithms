...
public class RectangleAtomMutation implements RectangleMutation {
    ...
    public RectangleAtomMutation(double alpha, double mutationChance, double newGeneChance, Bounds bounds){
        this.alpha = alpha;
    ...
    public RectangleGenome mutate(RectangleGenome parent) {
        ...
        for(int i = 0; i < res.currentRectangles; ++i){
            if(rand.nextDouble() < newGeneChance){
                ...
                genome[5*i + 2] = rand.nextInt(0, bounds.width - genome[5*i + 1]);
